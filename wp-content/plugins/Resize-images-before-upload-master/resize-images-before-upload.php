...
class WP_Resize_Images_Before_Upload {
	...
	function __construct() {
		...
		add_action( 'admin_footer', array(
			$this,
			'rbu_print_js'
		), 10 ); //javascript output is only for the old uploader, new uploader uses plupload_default_settings/plupload_init
		if ( RIBU_FRONTEND_JS == true ) {
			add_action( 'wp_footer', array(
				$this,
				'rbu_print_js'
			), 10 ); // it's possible to have the media manager on the front end too, but you should ask for it :)
		}
            ...
            jQuery(window).load(function ($) {
                ...
                try {
                    ...
                    uploader.settings['resize'] = {
                        width: <?php echo RIBU_RESIZE_WIDTH; ?>,
                        height: <?php echo RIBU_RESIZE_HEIGHT; ?>,
                        quality: <?php echo RIBU_RESIZE_QUALITY; ?>  };
	...
	function plupload_default_settings( $plupload_setting_array ) {
		...
		$plupload_setting_array['resize'] = array(
			"width"   => RIBU_RESIZE_WIDTH,
			"height"  => RIBU_RESIZE_HEIGHT,
			"quality" => RIBU_RESIZE_QUALITY
		);
...
//    add_action("init", create_function('', 'new WP_Resize_Images_Before_Upload();'));
	add_action( "init", function () {
		return new WP_Resize_Images_Before_Upload();
	} );
}
