...
//	}
	...
	wp_enqueue_script( 'lightgallery-script', get_template_directory_uri() . '/scripts/lightgallery.js', [], '1.0', true );
...
if ( wc_current_user_has_role( 'wc_product_vendors_admin_vendor' ) || wc_current_user_has_role( 'wc_product_vendors_manager_vendor' ) || wc_current_user_has_role( 'seller' ) ) {
	function photography_load_admin_scripts() {
		...
		global $typenow;
		if ( $typenow === "product" ) {
			wp_enqueue_style( 'admin-product-css', get_template_directory_uri() . '/assets/css/admin-product.css', false, '1.13' );
