...
if (!wc_current_user_has_role('administrator') && (wc_current_user_has_role('wc_product_vendors_admin_vendor') || wc_current_user_has_role('wc_product_vendors_manager_vendor') || wc_current_user_has_role('seller'))) {
    ...
    {
        wp_enqueue_style('admin_css', get_template_directory_uri() . '/assets/css/admin-style.css', false, '1.12');

        global $typenow;
        if ($typenow === "product") {
            wp_enqueue_style('admin-product-css', get_template_directory_uri() . '/assets/css/admin-product.css', false, '1.16');

            wp_enqueue_script('photography-admin-product-js', get_template_directory_uri() . '/scripts/admin-product.js', [], '1.05', true);
        }

        wp_enqueue_script('photography-admin-js', get_template_directory_uri() . '/scripts/admin.js', [], '1.03', true);
